...
<template><section class="surface overflow-hidden">
  <header class="flex flex-col gap-3 border-b border-[#e8ecef] p-4 lg:flex-row lg:items-center lg:justify-between"><div class="flex flex-1 flex-col gap-2 sm:flex-row"><div class="relative w-full max-w-sm"><PhMagnifyingGlass :size="18" class="absolute left-3 top-1/2 -translate-y-1/2 text-[#98a2b3]"/><input v-model="query" class="field pl-10" placeholder="Search loan or borrower"/></div><select v-model="status" class="field sm:!w-40" @change="load"><option value="">All statuses</option><option value="active">Active</option><option value="completed">Completed</option></select></div></header>
