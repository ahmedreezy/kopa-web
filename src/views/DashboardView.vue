...
<template>
  <div class="space-y-5">
    ...
    <section class="grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_300px]">
      ...
      <aside class="surface h-fit overflow-hidden"><header class="border-b border-[#e8ecef] px-5 py-4"><h2 class="font-semibold text-[#1d2939]">Quick records</h2><p class="mt-1 text-xs text-[#7d8796]">Common desk operations</p></header><nav class="divide-y divide-[#edf0f2]"><RouterLink to="/borrowers/new" class="flex items-center gap-3 px-5 py-4 text-sm font-medium text-[#475467] hover:bg-[#f7faf9] hover:text-[#287f71]"><PhPlus :size="18"/>Register borrower</RouterLink><RouterLink to="/collections" class="flex items-center gap-3 px-5 py-4 text-sm font-medium text-[#475467] hover:bg-[#f7faf9] hover:text-[#287f71]"><PhWallet :size="18"/>Record repayment</RouterLink></nav></aside>
    </section>
  </div>
</template>
